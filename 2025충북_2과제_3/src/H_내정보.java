...
public class H_내정보 extends BF {
	...
	public H_내정보() {
		setTitle("내 정보");
		setBounds(100, 100, 723, 496);
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		getContentPane().setLayout(null);
		
		label = new JLabel("");
		label.setBorder(new LineBorder(new Color(0, 0, 0)));
		label.setBounds(12, 10, 140, 127);
		getContentPane().add(label);
		
		label_1 = new JLabel("New label");
		label_1.setFont(new Font("맑은 고딕", Font.BOLD, 15));
		label_1.setBounds(165, 10, 209, 32);
		getContentPane().add(label_1);
		
		label_2 = new JLabel("New label");
		label_2.setFont(new Font("맑은 고딕", Font.BOLD, 15));
		label_2.setBounds(165, 61, 209, 32);
		getContentPane().add(label_2);
		
		panel_2 = new JPanel();
		panel_2.setBounds(0, 141, 707, 314);
		getContentPane().add(panel_2);
		panel_2.setLayout(null);
		
		scrollPane = new JScrollPane();
		scrollPane.setBounds(12, 10, 336, 290);
		...
		scrollPane_1.setBounds(360, 10, 335, 290);
	...
	private void load() {
		...
		int h =scrollPane.getHeight()/2-30;
		try (var rs = res("select * from fb join food using(f_no) where u_no = "+uno)) {
			int i = 0;
			while(rs.next()) {
				A_패널 pp =new A_패널(getIcon("foods/"+rs.getInt("f_no")+".jpg",100,100), rs.getString("f_name"));
			...
			i++;
			panel.setPreferredSize(new Dimension(0, h*(i/2)));
		} catch (SQLException e) {
			e.printStackTrace();
		}
		try (var rs = res("select * from reservation join movie using(m_no) where u_no ="+uno)) {
			int i = 0;
			while(rs.next()) {
				A_패널 pp =new A_패널(getIcon("movies/"+rs.getInt("m_no")+".jpg",100,100), rs.getString("m_name"));
				pp.label.setHorizontalAlignment(0);
				pp.label_1.setHorizontalAlignment(0);
				pp.setSize(w, h);
				pp.setLocation(w*(i%2), h*(i/2));
			...
			i++;
			panel_1.setPreferredSize(new Dimension(0, h*(i/2)));
		} catch (SQLException e) {
			e.printStackTrace();
		}
	}
