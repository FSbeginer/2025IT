...
public class C_자격증목록 extends BF {
	...
	public C_자격증목록(int cgno, String sql) {
		...
		panel_1 = new JPanel();
		panel_1.setBounds(347, 126, 521, 34);
		getContentPane().add(panel_1);
		panel_1.setLayout(new GridLayout(0, 6, 0, 0));
		
		label_7 = new RoundLabel("추천과정");
		label_7.setHorizontalAlignment(SwingConstants.CENTER);
		label_7.setBackground(Color.BLUE);
		label_7.setForeground(Color.white);
		panel_1.add(label_7);
		
		label_8 = new RoundLabel("IT");
		label_8.setHorizontalAlignment(SwingConstants.CENTER);
		label_8.setBackground(Color.WHITE);
		panel_1.add(label_8);
		
		label_9 = new RoundLabel("요리");
		label_9.setHorizontalAlignment(SwingConstants.CENTER);
		label_9.setBackground(Color.WHITE);
		panel_1.add(label_9);
		
		label_10 = new RoundLabel("봉사");
		label_10.setHorizontalAlignment(SwingConstants.CENTER);
		label_10.setBackground(Color.WHITE);
		panel_1.add(label_10);
		
		label_11 = new RoundLabel("항공");
		label_11.setHorizontalAlignment(SwingConstants.CENTER);
		label_11.setBackground(Color.WHITE);
		panel_1.add(label_11);
		
		label_12 = new RoundLabel("의학");
		...
		label_16 = new JLabel(getIcon("icon/logo.png",49,50));
		label_16.addMouseListener(new Label_16MouseListener());
	...
	private void load2() {
		...
		try (var rs = res("select *, count(*) cnt from course_registration join certi using(cno) join teacher t using(tno) where true "+where+" and cname like '%"+like+"%' group by cno, ratring order by cnt desc,cno limit 6;")) {
			...
			int i = 0;
			while(rs.next()) {
				C_갤러리패널 pp =new C_갤러리패널(getIcon("certification/"+rs.getInt("cno")+".png"), rs.getString("cname"), rs.getString("tname"), rs.getString("type")+"/"+rs.getString("type1"), rs.getString("cmicorgan"), rs.getString("cciorgan"));
	...
	private void load() {
		...
		try (var rs = res("select *, count(*) cnt from course_registration join certi using(cno) join teacher t using(tno) where true "+where+" and cname like '%"+like+"%'  group by cno, ratring order by cnt desc,cno limit 6;")) {
			...
			int i = 0;
			while(rs.next()) {
				C_리스트패널 pp =new C_리스트패널(getIcon("certification/"+rs.getInt("cno")+".png"), rs.getString("cname"), rs.getString("tname"), rs.getString("type")+"/"+rs.getString("type1"), rs.getString("cmicorgan"), rs.getString("cciorgan"));
				w=pp.getWidth(); h = pp.getHeight();
				...
				pp.button.addActionListener(new ActionListener() {
					@Override
					public void actionPerformed(ActionEvent e) {
						showPage(new H_상세내용(cno), "H_자격증상세내용");
					}
				});
	...
	private class Label_16MouseListener extends MouseAdapter {
		@Override
		public void mouseClicked(MouseEvent e) {
			showPage("A_메인");
		}
